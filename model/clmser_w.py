...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.kernels1 = self.create_weights(cfg['CHANNEL'], 3)
        self.kernels2 = self.create_weights(cfg['CHANNEL'], 3)
        self.DownBlocks = self._make_layers(ConvBlock, self.kernels1, cfg['STRIDE'], padding=1,
                                            activation='ReLU', norm=True)
        self.UpBlocks = self._make_layers(DeConvBlock, self.kernels2[1:], cfg['STRIDE'][1:], padding=1,
                                          activation='ReLU', norm=True)
        self.OutBlock = DeConvBlock(self.kernels2[0], stride=1, padding=1,
    ...
    def forward(self, inputs):
        ...
        for i in range(depth - 2, -1, -1):
            ...
            x = self.UpBlocks.__getitem__(i)(0.5*x + 0.5*features[i+1], output_size)
